...
async function getWeather(place) {
  try {
    const response = await fetch(
      `https://api.weatherapi.com/v1/current.json?key=7eb784e77fd045e3bc0164711240307&q=${place}`
    );
...
function displayData(data) {
  ...
  if (data.weather.toLowerCase().includes("sunny")) {
    container.style.backgroundImage =
      'url("https://miro.medium.com/v2/resize:fit:1400/0*SsC2eAfmZAmPVMUe.jpg")';
  } else if (data.weather.toLowerCase().includes("rain")) {
    container.style.backgroundImage =
      'url("https://c4.wallpaperflare.com/wallpaper/146/76/2/autumn-leaves-love-rain-wallpaper-preview.jpg")';
  } else if (data.weather.toLowerCase().includes("cloudy")) {
    container.style.backgroundImage =
      'url("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSBPwNC6rS6RF8vWHp8xr9GGEWRbQKJi6DCFg&s")';
    ...
    container.style.backgroundImage =
      'url("https://c4.wallpaperflare.com/wallpaper/814/791/615/ice-snow-snowfall-winter-wallpaper-preview.jpg")';
  } else if (data.weather.toLowerCase().includes("mist")) {
    container.style.backgroundImage =
      'url("https://images.nationalgeographic.org/image/upload/v1638884972/EducationHub/photos/blue-mist.jpg")';
  }
